...
describe("SandwichForm", () => {
  ...
  describe("with all additions checked", () => {
    it("shows maximum price", () => {
      ...
      expect(getByText("Готово 130")).toBeInTheDocument();
    })
  })
  ...
  describe("on sandwich submit", () => {
    it("passes constructed sandwich", () => {
      ...
      const { getByText } = render(<SandwichForm onSandwichCreated={onSandwichSubmit} />)
      
      fireEvent.click(getByText("майонез"))
      fireEvent.click(getByText("сыр чеддер"))
      fireEvent.click(getByText("Готово 50"))
